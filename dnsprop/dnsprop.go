...
func LookupDNS(serverIP, domain, recordType string, results chan<- map[string]string) {
	...
	default:
		results <- map[string]string{"serverIP": serverIP, "status": "❌ Unsupported record type"}
		return
	}

	if err != nil {
		results <- map[string]string{"serverIP": serverIP, "status": "❌"}

	} else {
		if len(records) == 0 {
			results <- map[string]string{"serverIP": serverIP, "status": "❌ No records found"}
			return
		}
		results <- map[string]string{"serverIP": serverIP, "status": "✅ [" + strings.Join(records, ", ") + "]"}
	}
}
